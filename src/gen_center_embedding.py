...
def generate_stimuli_data():
    nouns_vocab: list[str] = ["reporter", "senator", "president", "doctor", "lawyer", "artist", "banker", "student", "teacher", "soldier", "detective", "neighbor", "chef", "pilot", "baker", "driver", "nurse", "guard", "judge", "thief", "actor", "writer", "singer", "dancer"]
    verbs_vocab: list[str] = ["met", "attacked", "ignored", "visited", "called", "helped", "warned", "admired", "stopped", "questioned", "avoided", "praised", "noticed", "watched", "trusted"]
    ...
    for i in range(1, NUM_ITEMS + 1):
        n_list = random.sample(nouns_vocab, 4)
        v_list = random.sample(verbs_vocab, 3)

        roles = {"N1": n_list[0], "N2": n_list[1], "N3": n_list[2], "N4": n_list[3], "V1": v_list[0], "V2": v_list[1], "V3": v_list[2]}
        ...
        parts = {
            ...
            "N4": _make_np(roles["N4"], "N4"),
            "V1": _make_v(roles["V1"], "V1"),
            "V2": _make_v(roles["V2"], "V2"),
            "V3": _make_v(roles["V3"], "V3"),
            "WHO": _make_func("who", "WHO")
        }

        ce_tokens = (parts["N1"] + parts["WHO"] + parts["N2"] + parts["WHO"] + parts["N3"] + parts["V3"] + parts["V2"] + parts["V1"] + parts["N4"])
        rb_tokens = (parts["N3"] + parts["V3"] + parts["N2"] + parts["WHO"] + parts["V2"] + parts["N1"] + parts["WHO"] + parts["V1"] + parts["N4"])

        ce_head = ce_tokens[0].copy()
        ce_head["word"] = "The"
        ce_tokens[0] = ce_head
        rb_head = rb_tokens[0].copy()
        rb_head["word"] = "The"
        rb_tokens[0] = rb_head

        sentence_data.append({"item": i, "condition": "CE", "sentence": " ".join([t["word"] for t in ce_tokens])})
        sentence_data.append({"item": i, "condition": "RB", "sentence": " ".join([t["word"] for t in rb_tokens])})

        for idx, token in enumerate(ce_tokens, 1):
            word_data.append({"item": i, "condition": "CE", "word_position": idx, "word": token["word"], "region": token["label"]})
        for idx, token in enumerate(rb_tokens, 1):
            word_data.append({"item": i, "condition": "RB", "word_position": idx, "word": token["word"], "region": token["label"]})

    return pd.DataFrame(sentence_data), pd.DataFrame(word_data)


def write_latex(df_sent, output_path) -> None:
    with open(output_path, "w") as f:
        ...
        for item_id in items:
            sent_ce = df_sent[(df_sent["item"] == item_id) & (df_sent["condition"] == "CE")]["sentence"].values[0]
            sent_rb = df_sent[(df_sent["item"] == item_id) & (df_sent["condition"] == "RB")]["sentence"].values[0]

            f.write(f"  \\item a. {sent_ce} \\\\ b. {sent_rb}\n")
